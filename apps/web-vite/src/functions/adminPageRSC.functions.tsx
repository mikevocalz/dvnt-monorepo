...
export const loadAdminPageRSC = createServerFn({ method: 'GET' })
  ...
  // @ts-expect-error upstream Payload v4 tanstack-start adapter: ServerFn generic drift (build-verified)
  .handler(async ({ data }) => {
    ...
    const renderNotFound = async (): Promise<LoadResult> => {
      ...
      const notFoundNode = await renderNotFoundPage({
        ...
        // @ts-expect-error segments is intentionally string[]|undefined (see note above)
    ...
    try {
      ...
      const meta = await generatePageMetadata({
        // @ts-expect-error upstream Payload v4 adapter: AdminViewAdapter variance (build-verified)
        adminViews: defaultAdminViews,
        config,
        i18n,
        params: { segments },
