    ...
    try {
      ...
      for (const e of events.rows) {
        ...
        if (existing.docs[0]) {
          ...
          const created = await payload.create({ collection: 'events', data, overrideAccess: true })
      ...
      for (const t of tickets.rows) {
        ...
        if (existing.docs[0]) {
          ...
          await payload.update({ collection: 'tickets', id: existing.docs[0].id, data: appFields, overrideAccess: true })
          tUpdated++
        } else {
          await payload.create({
            collection: 'tickets',
            data: { ...appFields, attendeeName: t.attendee_name || t.guest_name || undefined, quantity: 1 },
            overrideAccess: true,
          })
          tCreated++
        }
