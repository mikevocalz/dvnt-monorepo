...
-- project (20260601202410 :: event_rpcs_return_flyer_image_url). This migration was applied to production
-- out-of-band and had no file in this directory; the SQL below is the
-- statement array the ledger recorded, so the repo now replays to the
-- schema production actually runs. Already recorded as applied.

-- Add flyer_image_url to the three event-listing RPCs so the client's
-- resolveFlyerVideoUrl() helper actually has data to inspect. Without
-- this column in the projection, every event card silently falls back
-- to the static cover image even when the host uploaded a video flyer.

-- ── 1. get_events_home (11-arg current version with p_nsfw) ──
...
-- ── 2. get_events_home (legacy 10-arg version) ──────────────
CREATE OR REPLACE FUNCTION public.get_events_home(
  p_limit integer DEFAULT 20,
  p_offset integer DEFAULT 0,
  p_viewer_id integer DEFAULT NULL,
