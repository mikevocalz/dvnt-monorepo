-- ============================================================
-- Extend event discovery RPCs:
--
-- 1. get_events_home:
--    a) Search now matches host username AND host full name
--       (from Better Auth "user" table) in addition to
--       title / description / location.
--    b) New p_nsfw boolean DEFAULT NULL parameter — when true,
--       returns only nsfw events; when false, hides them;
--       when NULL (default), returns all.
--
-- 2. Grants updated to include new signature.
-- ============================================================
...
BEGIN
  ...
  FROM (
    ...
    WHERE e.start_date IS NOT NULL
      ...
      -- nsfw filter: NULL = all, true = only nsfw, false = hide nsfw
...
REVOKE ALL ON FUNCTION public.get_events_home(integer, integer, integer, integer, boolean, boolean, boolean, text, text, text) FROM authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.get_events_home(integer, integer, integer, integer, boolean, boolean, boolean, text, text, text, boolean) TO authenticated, anon, service_role;
