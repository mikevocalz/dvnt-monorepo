-- Extend cart_complete_issuance to stamp group-order position + attendee name on
-- each child ticket (prompt 5.6.5a): order_index 1..N, order_count N, attendee_name
-- from the prepared row. Supersedes the 4-arg body in 20260613000300. Idempotent
-- (cart.status='completed' short-circuits). Applied to production 2026-06-13.
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO public.tickets (
        ...
        order_index, order_count, attendee_name
      ) VALUES (
        ...
        v_order_index, v_expected_count, NULLIF(v_prepared.attendee_name, '')
      );
