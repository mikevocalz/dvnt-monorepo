...
-- project (20260613183916 :: save_aggregate_tiers_addons). This migration was applied to production
    ...
    FOR v_a IN SELECT * FROM jsonb_array_elements(p_payload->'addons') LOOP
      ...
      IF v_exist IS NOT NULL THEN
        ...
        UPDATE public.ticket_addons SET
          ...
          is_redeemable=v_addon.is_redeemable, image_url=v_addon.image_url, sort_order=v_addon.sort_order, status=v_addon.status
        WHERE id = v_id;
        v_addon_id := v_id;
      ELSE
