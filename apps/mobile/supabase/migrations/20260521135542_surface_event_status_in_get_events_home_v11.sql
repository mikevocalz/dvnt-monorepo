...
-- project (20260521135542 :: surface_event_status_in_get_events_home_v11). This migration was applied to production
-- out-of-band and had no file in this directory; the SQL below is the
-- statement array the ledger recorded, so the repo now replays to the
-- schema production actually runs. Already recorded as applied.

-- The 11-arg overload of get_events_home (with p_nsfw) is the one the
-- client actually invokes. Earlier migration only patched the 10-arg
-- version, leaving e.status missing from feed payloads in production.

CREATE OR REPLACE FUNCTION public.get_events_home(
  p_limit integer DEFAULT 20,
  p_offset integer DEFAULT 0,
  p_viewer_id integer DEFAULT NULL::integer,
  p_city_id integer DEFAULT NULL::integer,
  p_filter_online boolean DEFAULT NULL::boolean,
  p_filter_tonight boolean DEFAULT false,
  p_filter_weekend boolean DEFAULT false,
  p_search text DEFAULT NULL::text,
  p_category text DEFAULT NULL::text,
  p_sort text DEFAULT 'soonest'::text,
  p_nsfw boolean DEFAULT NULL::boolean
)
 RETURNS json
 LANGUAGE plpgsql
