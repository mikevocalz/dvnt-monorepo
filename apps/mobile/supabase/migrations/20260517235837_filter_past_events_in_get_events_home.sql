...
-- project (20260517235837 :: filter_past_events_in_get_events_home). This migration was applied to production
-- out-of-band and had no file in this directory; the SQL below is the
-- statement array the ledger recorded, so the repo now replays to the
-- schema production actually runs. Already recorded as applied.

-- Filter out past events (>24h after start) from the home feed.
-- Previously the RPC returned past events when sort='soonest', causing
-- new upcoming events to sit below stale Feb–April events.
...
BEGIN
  ...
  FROM (
    ...
    WHERE e.start_date IS NOT NULL
      -- Hide events whose start_date is more than 24h in the past.
      -- end_date is preferred when present (events crossing midnight).
