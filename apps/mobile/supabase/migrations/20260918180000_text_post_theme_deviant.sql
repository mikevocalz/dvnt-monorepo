-- The client has always offered five text-post themes; the server accepted
-- four. `deviant` was missing from the CHECK constraint, from this RPC's
-- predicate, and from the create-post edge function's allowlist — so choosing
-- it fell through a silent `ELSE 'graphite'` at every layer. No error, no
-- warning: the post simply came back grey. Every text post in the table is
-- 'graphite' as a result.
--
-- Constraint first, then the RPC. The reverse order would let the RPC write a
-- value the table still rejects.
ALTER TABLE public.posts DROP CONSTRAINT IF EXISTS posts_text_theme_check;
ALTER TABLE public.posts ADD CONSTRAINT posts_text_theme_check
  CHECK (text_theme = ANY (ARRAY['graphite','deviant','cobalt','ember','sage']));

CREATE OR REPLACE FUNCTION public.create_post_with_dedupe(p_author_id integer, p_content text DEFAULT NULL::text, p_post_kind text DEFAULT 'media'::text, p_text_theme text DEFAULT NULL::text, p_location text DEFAULT NULL::text, p_is_nsfw boolean DEFAULT false, p_visibility text DEFAULT 'public'::text, p_slides text[] DEFAULT ARRAY[]::text[], p_media jsonb DEFAULT '[]'::jsonb, p_recent_window_seconds integer DEFAULT 90)
 RETURNS TABLE(id bigint, author_id integer, content text, post_kind text, text_theme text, location text, is_nsfw boolean, visibility text, created_at timestamp with time zone, was_created boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_text_theme text := CASE
    WHEN p_text_theme IN ('graphite', 'deviant', 'cobalt', 'ember', 'sage')
      THEN p_text_theme
    ELSE 'graphite'
  END;
...
BEGIN
  ...
  WHERE p.author_id = p_author_id
    ...
    AND COALESCE(p.visibility::text, 'public') = v_visibility
  ...
  IF FOUND THEN
    RETURN QUERY
    SELECT
      v_existing.id::bigint,
      v_existing.author_id,
      v_existing.content::text,
      v_existing.post_kind::text,
      v_existing.text_theme::text,
      v_existing.location::text,
      v_existing.is_nsfw,
      v_existing.visibility::text,
  ...
  VALUES (
    ...
    v_location,
    v_is_nsfw,
    v_visibility::public.enum_posts_visibility,
    0,
    0
  )
  RETURNING *
  ...
  RETURN QUERY
  SELECT
    v_inserted.id::bigint,
    v_inserted.author_id,
    v_inserted.content::text,
    v_inserted.post_kind::text,
    v_inserted.text_theme::text,
    v_inserted.location::text,
    v_inserted.is_nsfw,
    v_inserted.visibility::text,
