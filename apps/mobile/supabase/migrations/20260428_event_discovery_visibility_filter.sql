-- ============================================================
-- Enforce visibility filtering in event discovery RPCs.
--
-- Before this migration, all three visibility values
-- (public / link_only / private) appeared identically in
-- get_events_home and get_events_for_you. The UI copy now
-- promises that link_only and private events are hidden from
-- the public feed and search — this migration makes that true.
--
-- Rule applied to both RPCs:
--   COALESCE(e.visibility, 'public') = 'public'
--
-- COALESCE handles rows created before the visibility column
-- existed (NULL → treated as public, preserving old behaviour).
-- link_only and private rows are excluded from both feeds.
-- Hosts find their own non-public events via the organiser
-- dashboard / profile, not through discovery.
-- ============================================================
