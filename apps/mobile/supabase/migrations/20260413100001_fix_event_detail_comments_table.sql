-- Fix get_event_detail: two bugs from 20260413 migration:
-- 1. Wrong table name: post_comments → event_comments (post_comments doesn't exist)
-- 2. Missing columns in ticket_types: description, original_price_cents, is_sold_out,
--    max_per_order, tier, glow_color, perks, is_active
-- Both caused runtime errors → all events showed "Event Not Found".

-- Add missing ticket_types columns
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS description text;
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS original_price_cents integer;
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS is_sold_out boolean DEFAULT false;
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS max_per_order integer;
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS tier text DEFAULT 'ga';
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS glow_color text;
ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS perks jsonb DEFAULT '[]';
...
BEGIN
  ...
  -- Top comments (max 5) — uses event_comments table (NOT post_comments)
  LEFT JOIN LATERAL (
    SELECT json_agg(json_build_object(
      'id', c.id,
      'content', c.content,
