...
-- project (20260613145154 :: cart_issuance_addons). This migration was applied to production
-- out-of-band and had no file in this directory; the SQL below is the
-- statement array the ledger recorded, so the repo now replays to the
-- schema production actually runs. Already recorded as applied.

CREATE OR REPLACE FUNCTION public.cart_complete_issuance(
  p_cart_id uuid, p_payment_intent_id text, p_ticket_rows jsonb, p_addon_rows jsonb DEFAULT '[]'::jsonb
)
...
BEGIN
  ...
  IF v_prepared_count <> v_expected_count THEN
    RETURN jsonb_build_object('ok', false, 'error', 'prepared_ticket_count_mismatch',
      'expected', v_expected_count, 'actual', v_prepared_count); END IF;
  ...
  LOOP
    ...
    IF v_line_prepared_count <> v_line.quantity THEN
      RETURN jsonb_build_object('ok', false, 'error', 'prepared_line_item_count_mismatch',
        'lineItemId', v_line.line_item_id, 'expected', v_line.quantity, 'actual', v_line_prepared_count); END IF;
    FOR v_prepared IN
  ...
  LOOP
    ...
    WHERE prepared.line_item_id = v_addon_line.line_item_id LIMIT 1;
  ...
  UPDATE public.orders SET status = 'paid', stripe_payment_intent_id = p_payment_intent_id, paid_at = now(), updated_at = now()
