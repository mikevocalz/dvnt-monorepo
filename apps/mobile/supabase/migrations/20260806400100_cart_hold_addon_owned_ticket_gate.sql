-- ════════════════════════════════════════════════════════════════════════
-- cart_create_hold v4 — post-purchase add-on gating.
--
-- Base copied VERBATIM from 20260806201000 (v3: tier-visibility guards +
-- p_unlock_codes). ONE additive change, in the add-on loop:
--
--   The `requires_tier_id` gate previously required the gating tier to be a
--   line in THIS cart. That blocks the post-purchase upsell (ticket-upgrade /
--   ticket-detail "add more"), where the buyer already OWNS the qualifying
--   ticket and submits an add-on-only cart. The gate is now satisfied when
--   EITHER (a) the cart contains a line for the required tier (unchanged
--   checkout path), OR (b) the cart's user owns a live ticket
--   (status active/scanned) of the required tier for the same event.
--   Guest carts (user_id NULL) never match (b), so guests still need the
--   tier in-cart — no loosening of the guest path.
--
-- Signature unchanged (uuid, integer, jsonb) → plain CREATE OR REPLACE; the
-- v3 migration already dropped the legacy 2-arg overload. Error contract
-- unchanged ('addon_requires_tier' still returned when neither holds).
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    -- ── v4: requires_tier satisfied by an in-cart tier line OR a live
    --    owned ticket of that tier (post-purchase add-on-only carts). ──
    IF v_line.requires_tier_id IS NOT NULL
       ...
       AND NOT (
         v_cart.user_id IS NOT NULL
         AND EXISTS (
           SELECT 1 FROM public.tickets t
           WHERE t.user_id = v_cart.user_id
             AND t.event_id = v_cart.event_id
             AND t.ticket_type_id = v_line.requires_tier_id
             AND t.status IN ('active', 'scanned'))) THEN
...
-- Re-pin execution grants (mirrors 20260516150000: service_role only; the
-- RPC is reached exclusively through edge fns).
