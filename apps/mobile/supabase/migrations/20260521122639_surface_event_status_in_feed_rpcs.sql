...
-- project (20260521122639 :: surface_event_status_in_feed_rpcs). This migration was applied to production
-- out-of-band and had no file in this directory; the SQL below is the
-- statement array the ledger recorded, so the repo now replays to the
-- schema production actually runs. Already recorded as applied.

-- Surface `e.status` in the home + for-you feed RPCs so the client can
-- render a "CANCELLED" badge on event cards when an organizer cancels.
-- Both functions previously omitted status entirely; the row stayed in
-- the feed (no status filter in the WHERE clause) but the client had no
-- way to know which rows were cancelled.
