-- Discovery RPCs: a cancelled event is not discoverable
--
-- BUG: https://dvntapp.live/events/dc-dick-strict rendered two events. Two rows
-- share that title — 79 (status='cancelled') and 81 (active) — and the web slug
-- route resolves by slugify(title) over an unfiltered list, so the cancelled row
-- was offered at all. The same hole exists in the browse feeds: get_events_home
-- and get_events_for_you filter on visibility but never on status, so a cancelled
-- event stayed in the events tab, in search results and in For You. The client
-- only papered over it with a CANCELLED badge.
--
...
-- same SET search_path, same columns; this file is 20260904140100 plus that line.
--
-- NOT changed, on purpose — a cancelled event must stay reachable by ownership:
--   - get_event_detail / get_event_detail_v2: the detail screen a ticket holder
--     opens by id, and the cancelled-event takeover banner it renders.
--   - tickets/QR/wallet reads, refunds, payouts, event analytics, the host
--     dashboard and the attendee roster. One live cancelled event has 87 ticket
--     holders; every one of them must still be able to open what they hold.
--
-- Pairs with the client-side gate in packages/app/lib/events/event-discovery.ts,
-- which applies the same rule so a build running against a database without this
-- migration is still correct.
--
-- NOTE (pre-existing, NOT introduced here): the two get_events_home overloads are
-- ambiguous for a 10-argument call (the 11-arg overload defaults p_nsfw). Both are
-- updated identically, so behaviour is the same whichever the planner resolves.
--
-- DOWN: re-run 20260904140100_event_list_rpcs_video_flyer.sql verbatim.
