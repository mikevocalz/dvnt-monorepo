-- ============================================================
-- Re-add private event visibility enforcement to get_event_detail.
--
-- Migration 20260501100001 removed the visibility WHERE clause
-- (probably accidentally during an attendee-fields refactor),
-- making ALL private events readable by anyone with the event ID.
--
-- This migration restores the check and also extends it to allow
-- co-organizers and invited users (not just host/RSVP) to access
-- private events they are associated with.
--
-- Access rule for visibility = 'private':
--   viewer must be one of:
--     1. The event host  (host_id = viewer.auth_id)
--     2. Has RSVP'd      (event_rsvps)
--     3. Is a co-organizer (event_co_organizers)
--     4. Has an invite   (event_invites)
--
-- link_only and public remain open to anyone with the event ID.
-- ============================================================
...
BEGIN
  ...
  FROM (
    ...
    WHERE e.id = p_event_id
      -- private events require an explicit relationship with the viewer
      AND (
        COALESCE(e.visibility, 'public') != 'private'
        OR p_viewer_id IS NULL  -- will return no row (visibility check fails)
        OR EXISTS (
