-- Order the "who's going" attendee slice so users who actually have an
-- avatar come first. The old `merged LIMIT 20` had no ORDER BY, so the
-- face pile was an arbitrary 20 of the attendee set and usually landed on
-- avatar-less accounts even when real attendee photos existed.
