...
// Headless Payload REST client. The console is served by the same Next app as
// the Payload API (mounted at /payload-api — see src/payload.config.ts
// routes.api), so requests are same-origin with cookie auth — no CORS, no token
// plumbing. Override with NEXT_PUBLIC_PAYLOAD_URL only if the console is ever
// hosted on a different origin than Payload.
