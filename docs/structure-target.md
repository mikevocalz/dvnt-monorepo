...
4. **Store rules** — Zustand only for app/business state (per `AGENTS.md`/`CLAUDE.md` I-list), MMKV persistence, `useState` for local UI ephemera only, stores live in the feature's `stores/` (or `lib/stores/` when truly global like `auth-store`/`ui-store`).
