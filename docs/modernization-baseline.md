...
Toolchain: pnpm 10.32.1, turbo 2.9.17. Repo is Expo **SDK 57** (`expo ~57.0.1`, RN 0.86, React 19.2.3), Next **16.1.6**, Payload **4.0.0-internal.a0ef1b8**. (`AGENTS.md` says SDK 56 — stale; package.json is authoritative.)
...
It's a **Payload v4 ESM CLI / migration runner** — `src/` is just `payload.config.ts` (`export { default } from '@dvnt/cms'`) + `environment.d.ts`; the real work is `scripts/` (seed-admins, migrate-media-to-s3, etc.) and the `payload`/`migrate`/`generate:types`/`generate:importmap` npm scripts. It's the **only ESM context** that can run Payload's CLI (apps/web is CommonJS and can't). Import-orphaned as a library; nothing depends on it. Root `vercel.json` builds only `apps/web`. A separate `admin.dvnt.app` project is *documented* (`docs/vercel-deployment.md`) with a checked-in `dist/`, but live status is unconfirmed.
**Questions for you:** (1) Is `admin.dvnt.app` a live deployment or dead docs? (2) If web-vite is retired, **where do Payload migrations / type + importMap generation / seeding run instead?** They have no other home today. — Your call; nothing here touches it.
...
1. **`admin.dvnt.app` / web-vite fate** (§5A) — live or dead? If retired, where do Payload migrate/generate/seed run?
