...
6. **RESOLVED 2026-08-07 (live-DB check): the capacity-below-sold guards ARE applied in production** — `trg_event_capacity_guard` on `events` and `trg_tier_capacity_guard` on `ticket_types` both exist live, and `20260613001000` is in the migration ledger; the file's "NOT YET APPLIED TO PRODUCTION — awaiting Wave-2 authorization" header is stale. The Wave-2 stop-and-ask item is moot. Residual truth: `20260614234005` (promo BOGO) was applied out-of-band and back-recovered; `20260805130000_reconcile_prod_function_drift.sql` exists — prod/migration drift remains a live phenomenon, which is why the 2026-08-07 live validation pass (below) was run before any new push.
