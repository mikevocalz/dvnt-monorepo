...
| Boost billing route | Native IAP vs web Stripe is a business and App Review decision, not an engineering one. `types.ts` records that subscriptions dodge IAP via web-only sale; whether a boost is the same kind of good needs a written call. |
