...
export const FeedEventCard = memo(function FeedEventCard({
  ...
  // Hero placeholder color (Phase 4: never an empty #111). db value if set, else
  // extracted on-device; degrades to the brand ink if extraction can't run.
  const { color: heroBg } = useEventDominantColor({
    eventId: event.id,
    dominantColor: (event as any).dominantColor,
    imageUrl: event.flyerVideoUrl ? undefined : event.image,
    videoUrl: event.flyerVideoUrl,
  });
