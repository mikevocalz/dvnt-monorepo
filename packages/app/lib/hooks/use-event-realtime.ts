...
export function useEventsFeedRealtime(enabled = true): void {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      // New events published elsewhere → surface them in the browse lists.
      // Invalidate (re-run the RPC) so server-side ordering/filters apply,
      // rather than guessing where to splice a raw row into the cache.
      .on(
        "postgres_changes",
        {
          event: "INSERT",
