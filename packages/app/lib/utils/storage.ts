...
export const storage: StateStorage = {
  getItem: (name: string): string | null => {
    try {
      if (Platform.OS === "web") {
        // SSR: react-native-web reports OS "web" on the Next server, but there's
        // no localStorage there — return null silently (the store rehydrates on
        // the client). Avoids noisy SSR "[Storage] error" logs.
  ...
  setItem: (name: string, value: string): void => {
    try {
      if (Platform.OS === "web") {
        if (typeof localStorage === "undefined") return; // SSR — no-op
