...
import { authClient, handleSignOut, type AppUser } from "../auth-client";
import { auth } from "../api/auth";
import { logAuth } from "../auth/auth-logger";
  ...
  persist(
    (set, get) => ({
      ...
      logout: async () => {
        ...
        clearAuthStorage();
        clearUserDataFromStorage();
        const { clearUserRowCache } = require("../auth/identity");
        clearUserRowCache();
      },
      ...
      loadAuthState: async () => {
        ...
        try {
          ...
          if (session?.user) {
            ...
            try {
              const { syncAuthUser } = require("../api/privileged");
              let syncedUser;
              try {
