/**
 * EjectModal — NATIVE. Gorhom bottom sheet, non-dismissable by design.
 *
 ...
 *
 * Copy comes from EJECT_COPY so the web leg cannot drift from it.
 */
...
export function EjectModal({ visible, kind, reason, onDismiss }: EjectModalProps) {
  ...
  const copy = kind ? EJECT_COPY[kind] : null;
  ...
  // inner kind-dependent body when there's nothing to show.
  const shouldRenderBody = !!copy;

  return (
    <BottomSheet
      ref={sheetRef}
      index={visible && kind ? 0 : -1}
      animateOnMount
      enableDynamicSizing
