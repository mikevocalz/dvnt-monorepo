...
export function createWatchVenueWeatherLoader(fetcher: (opts: { lat: number; lng: number; shouldContinue: () => boolean }) => Promise<LiveSurfacePayload | null>) {
    ...
    if (!cache || now - cache.attemptedAt >= 15 * 60_000) {
      cache = { generation, eventId: focus.id, attemptedAt: now, result: (async () => {
        ...
        const payload = await Promise.race([
          fetcher({ lat: focus.latitude!, lng: focus.longitude!, shouldContinue }),
        ...
        return { tempF: weather.tempF, label: typeof weather.label === "string" ? weather.label.slice(0, 80) : undefined, generatedAt: new Date(stamp).toISOString(), precipPct: typeof weather.precipPct === "number" && weather.precipPct >= 0 && weather.precipPct <= 100 ? weather.precipPct : undefined };
      })().catch(() => undefined) };
    }
    const current = cache;
